...
{
public:
  EdgeHierarchyConstruction(EdgeHierarchyGraph& g, EdgeHierarchyQuery& query)
    : g(g)
    , query(query)
    , edgeRanker(g)
    , bipartiteMVC(g.getNumberOfNodes())
  {}
  ...
  void setEdgeRank(NODE_T u, NODE_T v, EDGERANK_T level)
  {
    ...
    pair<vector<pair<NODE_T, NODE_T>>,
      ...
      shortestPathsLost = getShortestPathsLost<true>(u, v, uVWeight, g, query);

    for (auto edgeToDecrease : shortestPathsLost.second) {
      g.decreaseEdgeWeight(
        get<0>(edgeToDecrease), get<1>(edgeToDecrease), get<2>(edgeToDecrease));
      if (g.getEdgeRank(get<0>(edgeToDecrease), get<1>(edgeToDecrease)) <
          EDGERANK_INFINIY) {
        g.setEdgeRank(
          get<0>(edgeToDecrease), get<1>(edgeToDecrease), EDGERANK_INFINIY);
    ...
    auto shortcutVertices =
      bipartiteMVC.getMinimumVertexCover(shortestPathsLost.first);
    ...
    //        assert(getShortestPathsLost<true>(u, v, uVWeight, g,
    //        query).first.size() == 0); assert(getShortestPathsLost<true>(u, v,
    //        uVWeight, g, query).second.size() == 0);
  }
