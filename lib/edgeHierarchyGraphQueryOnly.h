...
{
public:
  EdgeHierarchyGraphQueryOnly(NODE_T n)
    : n(n)
    , m(0)
    , neighborsOut(n)
    , neighborsIn(n)
  ...
  {
    if (edgesSorted) {
      std::cout << "Error. Trying change edge rank after sorting edges"
                << std::endl;
      exit(1);
    }
  ...
  {
    const size_t end =
      inBegin[v] + (((inBegin[v + 1] - inBegin[v]) * percent) / 100);
  ...
  {
    const size_t end =
      outBegin[v] + (((outBegin[v + 1] - outBegin[v]) * percent) / 100);
  ...
  template<typename F>
  void forAllNeighborsInWithHighRank(const NODE_T v,
                                     const EDGERANK_T _rankThreshold,
                                     F&& callback)
  {
  ...
  template<typename F>
  void forAllNeighborsOutWithHighRank(const NODE_T v,
                                      const EDGERANK_T _rankThreshold,
                                      F&& callback)
  {
    const EDGERANK_T rankThreshold = _rankThreshold;
