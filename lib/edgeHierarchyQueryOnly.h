...
template<bool stallForward,
         bool stallBackward,
         bool partialStalling,
         bool logVerticesSettled>
...
public:
  ...
  EdgeHierarchyQueryOnly(EdgeHierarchyGraphQueryOnly& g)
    : g(g)
                           ...
                           float stallingPercent)
  {
    ...
    while (!finished) {
      ...
      if (forward) {
        makeStep<true>(
          shortestPathMeetingNode, shortestPathLength, stallingPercent);
      } else {
        makeStep<false>(
          shortestPathMeetingNode, shortestPathLength, stallingPercent);
      }
...
protected:
  ...
  {
    const RoutingKit::TimestampFlags& wasPushedCurrent =
      forward ? wasPushedForward : wasPushedBackward;
    const vector<EDGEWEIGHT_T>& tentativeDistanceCurrent =
  ...
  bool canStallAtNodeForward(NODE_T v)
  {
    vector<EDGEWEIGHT_T>& tentativeDistanceCurrent =
      forward ? tentativeDistanceForward : tentativeDistanceBackward;
    vector<EDGEWEIGHT_T>& actualDistanceCurrent =
      forward ? actualDistanceForward : actualDistanceBackward;
    RoutingKit::TimestampFlags& actualDistanceSetCurrent =
      forward ? actualDistanceSetForward : actualDistanceSetBackward;
                ...
                int stallingPercent)
  {
    ...
    //         int stallingPercentThisIteration = (1.0 *
    //         numVerticesSettledThisQuery)/avgSearchSpace * stallingPercent;
    //         int stallingPercentThisIteration = (1.0 * (avgSearchSpace -
    //         numVerticesSettledThisQuery))/avgSearchSpace * stallingPercent;
    //         stallingPercentThisIteration =
    //         std::clamp(stallingPercentThisIteration, 0, 100);
    //         if(canStallAtNodeBackwardPartial<forward>(u,
    //         stallingPercentThisIteration)) {
    //             return;
    //         }
    //     }
    ...
    auto relaxFunc =
      [&](const NODE_T v, const EDGERANK_T rank, const EDGEWEIGHT_T weight) {
        ++numEdgesRelaxed;
        ...
        if (wasPushedCurrent.is_set(v)) {
          if (distanceV < tentativeDistanceCurrent[v]) {
            if constexpr (stallForward) {
              if (!actualDistanceSetCurrent.is_set(v) ||
                  distanceV < actualDistanceCurrent[v]) {
