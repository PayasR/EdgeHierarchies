...
public:
  ...
  pair<vector<NODE_T>, vector<NODE_T>> getMinimumVertexCover(
    vector<pair<NODE_T, NODE_T>>& edges)
  {
    ...
    assert(NODE_T(count_if(matchingPartnerLhs.begin(),
                           matchingPartnerLhs.begin() + nLhs,
                           [](NODE_T partner) {
                             return partner != NODE_INVALID;
  ...
  NODE_T getMinimumVertexCoverSize(vector<pair<NODE_T, NODE_T>>& edges)
  {
    ...
    NODE_T size =
      count_if(matchingPartnerLhs.begin(),
               matchingPartnerLhs.begin() + nLhs,
               [](NODE_T partner) { return partner != NODE_INVALID; });
