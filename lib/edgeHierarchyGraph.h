...
public:
  ...
  NODE_T getNumberOfNodes() { return n; }

  EDGECOUNT_T getNumberOfEdges() { return m; }
  ...
  NODE_T getInDegree(NODE_T v) { return neighborsIn[v].size(); }

  NODE_T getOutDegree(NODE_T v) { return neighborsOut[v].size(); }
  ...
  template<typename F>
  void forAllNeighborsInWithHighRank(NODE_T v,
                                     EDGERANK_T rankThreshold,
                                     F&& callback)
  {
    for (size_t i = 0; i < neighborsIn[v].size(); ++i) {
      if (neighborsIn[v][i].rank >= rankThreshold) {
        callback(neighborsIn[v][i].neighbor,
                 neighborsIn[v][i].rank,
                 neighborsIn[v][i].weight);
      } else if (edgesSorted) {
        break;
      }
    }
  ...
  template<typename F>
  void forAllNeighborsOutWithHighRank(NODE_T v,
                                      EDGERANK_T rankThreshold,
                                      F&& callback)
  {
    for (size_t i = 0; i < neighborsOut[v].size(); ++i) {
      if (neighborsOut[v][i].rank >= rankThreshold) {
        callback(neighborsOut[v][i].neighbor,
                 neighborsOut[v][i].rank,
                 neighborsOut[v][i].weight);
      } else if (edgesSorted) {
        break;
      }
    }
  ...
  {
    for (NODE_T v = 0; v < n; ++v) {
      sort(neighborsOut[v].begin(),
           neighborsOut[v].end(),
           [&](edgeInfo i, edgeInfo j) { return i.rank > j.rank; });
      sort(neighborsIn[v].begin(),
           neighborsIn[v].end(),
           [&](edgeInfo i, edgeInfo j) { return i.rank > j.rank; });
    }
  ...
  EdgeHierarchyGraph getTurnCostGraph(unsigned uTurnCost)
  {
    ...
    outDegree[0] = 0;
    forAllNodes([&](NODE_T v) { outDegree[v + 1] = getOutDegree(v); });
  ...
  T getDFSOrderGraph()
  {
    ...
    assert(dfsCount == n);
    if (dfsCount != n) {
      std::cout << "Error! dfsCount " << dfsCount << " instead of " << n
                << std::endl;
      exit(1);
    }
