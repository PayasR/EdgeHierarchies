...
RoutingKit::ContractionHierarchy
getCHFromGraph(EdgeHierarchyGraph& g)
{
  ...
  return RoutingKit::ContractionHierarchy::build(
    g.getNumberOfNodes(), tails, heads, weights);
}

template<class EdgeRanker>
void
buildAndWriteEdgeHierarchy(EdgeHierarchyGraph& g,
                           std::string edgeHierarchyFilename)
{
  ...
  cout << "Distance in Query graph was equal to removed path " << numEquals
       << " times" << endl;
...
std::vector<DijkstraRankRunningtime>
GenerateDijkstraRankQueries(unsigned numSourceNodes,
                            int seed,
  ...
  for (unsigned i = 0; i < numSourceNodes; ++i) {
    ...
    while (!dij.is_finished()) {
      ...
      if (n == (1u << r)) {
        ...
        if (r > 5) {
          result.push_back(
            { source_node, x.node, r, x.distance, -1, -1, -1, -1 });
        }
...
std::vector<DijkstraRankRunningtime>
GenerateRandomQueries(unsigned numQueries, int seed, EdgeHierarchyGraph& g)
{
...
template<bool partialStalling,
         bool EHForwardStalling,
         bool EHBackwardStalling,
         bool CHStallOnDemand,
         bool minimalSearchSpace,
         template<bool, bool, bool, bool>
         class QueryType>
int
benchmark(bool dijkstraRank,
          bool test,
          EdgeHierarchyGraphQueryOnly& ehGraph,
          RoutingKit::ContractionHierarchyQuery& chQuery,
          std::vector<DijkstraRankRunningtime>& queries,
          int stallingPercent)
{
  QueryType<EHForwardStalling,
            EHBackwardStalling,
            partialStalling,
            minimalSearchSpace>
    newQuery = QueryType<EHForwardStalling,
                         EHBackwardStalling,
                         partialStalling,
                         minimalSearchSpace>(ehGraph);
  // newQuery.avgSearchSpace = 626;
  // EdgeHierarchyQueryOnly<EHForwardStalling, EHBackwardStalling,
  // minimalSearchSpace> newQuery = EdgeHierarchyQueryOnly<EHForwardStalling,
  // EHBackwardStalling, minimalSearchSpace>(ehGraph);
  ...
  if (test) {
    for (auto& generatedQuery : queries) {
      ...
      chQuery.reset()
        .add_source(u)
        .add_target(v)
        .run<CHStallOnDemand, minimalSearchSpace>();
      ...
      if (generatedQuery.distance != distance) {
        cout << "EH: Wrong distance for " << u << " and " << v << ": "
             << distance << " (should be " << generatedQuery.distance << ")"
             << endl;
        numMistakes++;
      } else {
        numCorrect++;
      ...
      if (generatedQuery.distance != chDistance) {
        cout << "CH: Wrong distance for " << u << " and " << v << ": "
             << chDistance << " (should be " << generatedQuery.distance << ")"
             << endl;
      } else {
      }
    }
  ...
  for (auto& generatedQuery : queries) {
    ...
    if (dijkstraRank) {
      generatedQuery.timeEH =
        chrono::duration_cast<chrono::nanoseconds>(queryEnd - queryStart)
          .count();
      generatedQuery.verticesSettledEH = newQuery.numVerticesSettled;
      generatedQuery.edgesRelaxedEH =
        newQuery.numEdgesRelaxed + newQuery.numEdgesLookedAtForStalling;
    }

    if constexpr (minimalSearchSpace) {
      for (std::pair<NODE_T, EDGEWEIGHT_T> nodeDistancePair :
           newQuery.verticesSettledForward) {
      ...
      for (std::pair<NODE_T, EDGEWEIGHT_T> nodeDistancePair :
           newQuery.verticesSettledBackward) {
  ...
  if (!dijkstraRank) {
    ...
    if constexpr (minimalSearchSpace) {
      cout << "MINIMAL average number of vertices settled (EH): "
           << numVerticesSettledWithActualDistance / queries.size() << endl;
    }
    cout << "Average number of vertices settled (EH): "
         << newQuery.numVerticesSettled / queries.size() << endl;
    cout << "Average number of edges relaxed (EH): "
         << newQuery.numEdgesRelaxed / queries.size() << endl;
    cout << "Average number of edges looked at for stalling (EH): "
         << newQuery.numEdgesLookedAtForStalling / queries.size() << endl;
  }
  ...
  for (auto& generatedQuery : queries) {
    ...
    auto queryEnd = chrono::high_resolution_clock::now();
    if (dijkstraRank) {
      generatedQuery.timeCH =
        chrono::duration_cast<chrono::nanoseconds>(queryEnd - queryStart)
          .count();
      generatedQuery.verticesSettledCH = chQuery.getNumVerticesSettled();
      generatedQuery.edgesRelaxedCH =
        chQuery.getNumEdgesRelaxed() + chQuery.getNumEdgesLookedAtForStalling();
    }
    if constexpr (minimalSearchSpace) {
      ...
      for (const std::pair<unsigned, unsigned>& nodeDistancePair :
           verticesSettledForward) {
        unsigned actualDistance =
          newQuery.getDistance(u, nodeDistancePair.first, stallingPercent);
        if (actualDistance == nodeDistancePair.second) {
          ++numVerticesSettledWithActualDistance;
        }
        if (actualDistance > nodeDistancePair.second) {
          std::cout << "IMPOSSIBLE!!! Distance from " << u << " to "
                    << nodeDistancePair.first << " is " << actualDistance
                    << " but was settled at " << nodeDistancePair.second
                    << std::endl;
        }
      }
      for (const std::pair<unsigned, unsigned>& nodeDistancePair :
           verticesSettledBackward) {
        unsigned actualDistance =
          newQuery.getDistance(nodeDistancePair.first, v, stallingPercent);
        if (actualDistance == nodeDistancePair.second) {
          ++numVerticesSettledWithActualDistance;
        }
        if (actualDistance > nodeDistancePair.second) {
          std::cout << "IMPOSSIBLE!!! Distance from " << nodeDistancePair.first
                    << " to " << v << " is " << actualDistance
                    << " but was settled at " << nodeDistancePair.second
                    << std::endl;
        }
      }
  ...
  if (dijkstraRank) {
    ...
    for (auto& generatedQuery : queries) {
      ...
      std::cout << "result EH: " << rank << " " << timeEH << " "
                << numVerticesSettledEH << " " << numEdgesRelaxedEH
                << std::endl;
      std::cout << "result CH: " << rank << " " << timeCH << " "
                << numVerticesSettledCH << " " << numEdgesRelaxedCH
                << std::endl;
    }
  }
          ...
          int stallingPercent)
{
  if (stallingPercent == -1) {
    return benchmark<false,
                     ...
                     minimalSearchSpace,
                     QueryType>(
      dijkstraRank, test, ehGraph, chQuery, queries, stallingPercent);
  } else {
          ...
          int stallingPercent)
{
  if (noTimestamp) {
    return -1;
    // return benchmark<EHForwardStalling, EHBackwardStalling, CHStallOnDemand,
    // minimalSearchSpace, EdgeHierarchyQueryOnlyNoTimestamp>(dijkstraRank,
    // test, ehGraph, chQuery, queries, stallingPercent);
  } else
    return benchmark<EHForwardStalling,
                     EHBackwardStalling,
                     CHStallOnDemand,
                     minimalSearchSpace,
                     EdgeHierarchyQueryOnly>(
      dijkstraRank, test, ehGraph, chQuery, queries, stallingPercent);
}

template<bool EHForwardStalling, bool EHBackwardStalling, bool CHStallOnDemand>
int
benchmark(bool minimalSearchSpace,
          bool dijkstraRank,
          bool test,
          bool noTimestamp,
          EdgeHierarchyGraphQueryOnly& ehGraph,
...
int
benchmark(bool CHStallOnDemand,
          bool minimalSearchSpace,
          bool dijkstraRank,
          bool test,
          bool noTimestamp,
...
{
  if (CHStallOnDemand)
    return benchmark<EHForwardStalling, EHBackwardStalling, true>(
      minimalSearchSpace,
      dijkstraRank,
      test,
      noTimestamp,
    ...
    return benchmark<EHForwardStalling, EHBackwardStalling, false>(
      minimalSearchSpace,
      dijkstraRank,
      test,
      noTimestamp,
...
template<bool EHForwardStalling>
int
benchmark(bool EHBackwardStalling,
          bool CHStallOnDemand,
          bool minimalSearchSpace,
          bool dijkstraRank,
          bool test,
          ...
          int stallingPercent)
{
  if (EHBackwardStalling)
    return benchmark<EHForwardStalling, true>(CHStallOnDemand,
                                              minimalSearchSpace,
                                              dijkstraRank,
                                              test,
                                              noTimestamp,
    ...
    return benchmark<EHForwardStalling, false>(CHStallOnDemand,
                                               minimalSearchSpace,
                                               dijkstraRank,
                                               test,
                                               noTimestamp,
...
int
benchmark(bool EHForwardStalling,
          bool EHBackwardStalling,
          bool CHStallOnDemand,
          bool minimalSearchSpace,
          bool dijkstraRank,
          ...
          int stallingPercent)
{
  if (EHForwardStalling)
    return benchmark<true>(EHBackwardStalling,
                           CHStallOnDemand,
                           minimalSearchSpace,
                           dijkstraRank,
                           test,
    ...
    return benchmark<false>(EHBackwardStalling,
                            CHStallOnDemand,
                            minimalSearchSpace,
                            dijkstraRank,
                            test,
...
main(int argc, char* argv[])
{
  ...
  cp.add_unsigned('q', "queries", "N", numQueries, "Run N queries.");

  unsigned seed = 0;
  cp.add_unsigned('s', "seed", "N", seed, "The seed to use for the prng.");

  std::string filename;
  cp.add_param_string(
    "filename", filename, "Filename of the graph to benchmark on");
  ...
  cp.add_unsigned('u',
                  "uTurnCost",
                  uTurnCost,
                  "Cost to be added for u-turns (only has effect when turn "
                  "costs are activated).");

  bool useCHForEHConstruction = false;
  cp.add_bool(
    "useCH",
    useCHForEHConstruction,
    "If this flag is set, CH queries will be used during EH construction");

  bool DFSPreOrder = false;
  cp.add_bool("DFSPreOrder",
              DFSPreOrder,
              "If this flag is set, DFS ordering will use pre order instead of "
              "post order");
  ...
  cp.add_bool("noTimestamp",
              noTimestamp,
              "If this flag is set, EH queries will not use timestamp flags "
              "but instead reset all distances set after each query");

  bool dijkstraRank = false;
  cp.add_bool('d',
              "dijkstraRank",
              dijkstraRank,
              "If this flag is set, queries are generated for dijkstra ranks "
              "of powers of two with numQueries source vertices.");
  ...
  cp.add_int("partialStallingPercent",
             partialStallingPercent,
             "Determines the fraction of incoming edges looked at for backward "
             "stalling in percent. Set -1 to disable partial stalling, -2 for "
             "scaling in increments of 10%. (default: -1)");
  ...
  cp.add_bool("CHNoStallOnDemand",
              CHNoStallOnDemand,
              "If this flag is set, Contraction Hierarchy queries will NOT use "
              "stall on demand");
  ...
  bool test = false;
  cp.add_bool("test", test, "If this flag is set, correctness will be checked");

  bool rebuild = false;
  cp.add_bool("rebuild", rebuild, "If this flag is set, CH and EH are rebuilt");
  ...
  if (!rebuild && fileExists(edgeHierarchyFilename) &&
      fileExists(contractionHierarchyFilename) && !dijkstraRank) {
    std::cout << "Skip reading graph file because both EH and CH are already "
                 "on disk and dijkstraRank is deactivated"
              << std::endl;
  } else {
    auto start = chrono::steady_clock::now();
    ...
    cout << "Input graph has " << g.getNumberOfNodes() << " vertices and "
         << g.getNumberOfEdges() << " edges" << endl;
    ...
    if (addTurnCosts) {
      ...
      cout << "Turn cost graph has " << g.getNumberOfNodes() << " vertices and "
           << g.getNumberOfEdges() << " edges" << endl;
    }
  ...
  if (!rebuild && fileExists(contractionHierarchyFilename)) {
    std::cout << "Contraction Hierarchy already stored in file. Loading it..."
              << std::endl;
    ch =
      RoutingKit::ContractionHierarchy::load_file(contractionHierarchyFilename);
  } else {
  ...
  cout << "CH has "
       << ch.forward.first_out.back() + ch.backward.first_out.back() << " edges"
       << endl;

  shortcutHelperChQuery = chQuery;

  if (!rebuild && fileExists(edgeHierarchyFilename)) {
    std::cout << "Edge Hierarchy already stored in file. Loading it..."
              << std::endl;
    g = readEdgeHierarchy(edgeHierarchyFilename);
  } else {
    std::cout << "Building Edge Hierarchy..." << std::endl;
    buildAndWriteEdgeHierarchy<ShortcutCountingRoundsEdgeRanker>(
      g, edgeHierarchyFilename);
  }
  g.sortEdges();
  cout << "Edge hierarchy graph has " << g.getNumberOfNodes()
       << " vertices and " << g.getNumberOfEdges() << " edges" << endl;
  EdgeHierarchyGraphQueryOnly newG(0);
  if (CHOrder) {
    newG = g.getReorderedGraph<EdgeHierarchyGraphQueryOnly>(ch.rank);
    cout << "Reordered edge hierarchy graph has " << newG.getNumberOfNodes()
         << " vertices and " << newG.getNumberOfEdges() << " edges" << endl;
  } else {
    if (DFSPreOrder) {
    ...
    cout << "DFS ordered edge hierarchy graph has " << newG.getNumberOfNodes()
         << " vertices and " << newG.getNumberOfEdges() << " edges" << endl;
  }
